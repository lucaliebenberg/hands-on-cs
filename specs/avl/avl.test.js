...
class Node {
  ...
  balance() {
    ...
    // if it is, do I need to single or double rotate
    // if single, just call rotate on self
    // if double, call rotate on child and then self
  ...
  updateInNewLocation() {
    ...
    if (!this.right && !this.left) {
      ...
      this.height = this.left.height + 1; // if left height is greater
    } else {
      this.height = this.right.height + 1; // if right height is greater
    }
  }
}
